...
const AddStudent = () => {
  ...
  // Get today's date in YYYY-MM-DD format for the max dob attribute
      ...
      <div className="bg-white w-full max-w-4xl rounded-2xl shadow-lg border border-gray-100 overflow-hidden mt-2 sm:mt-4">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-700 p-5 sm:p-6 text-white relative">
          ...
          <div className="relative z-10">
            ...
            <p className="mt-1 text-indigo-100 text-sm font-medium">Register a new student into the system</p>
          </div>
        </div>
...
const Input = ({
  ...
  <div className="flex flex-col gap-1.5">
    <label className="text-xs font-bold text-gray-700 flex justify-between">
      {label}
      {readOnly && (
        <span className="text-[9px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full uppercase tracking-wider font-extrabold border border-gray-200">Auto</span>
      )}
    </label>
    <div className="relative">
      {icon && (
