...
export default function StudentSidebar({ studentEmail }) {
  ...
  useEffect(() => {
    fetchUnseen();
    const interval = setInterval(fetchUnseen, 20000); // Poll every 20s
    return () => clearInterval(interval);
  }, [studentEmail]);

  /* ================= MARK NOTIFICATIONS AS SEEN WHEN TAB OPEN ================= */
  useEffect(() => {
    const markSeen = async () => {
      if (location.pathname === "/studentdashboard/notifications" && hasNewNotification) {
        try {
          await axios.post(
